...
\begin{enumerate}
\item Normalized Document Length (NDL)
...
$\text{Normalized Document Length} (NDL)=\dfrac{\text{Document Length}} {\text{Maximum Document Length in the dataset}}$
...
\item Normalized Term Frequency (NTF)
...
$\text{Normalized Term Frequency} (NTF)=	1	+\log	$(TF of person entity in current article)


\item Number of similar articles (NSIM)
...
Among the top 30, 16 of the person entities were found to be influential and popular in the 19th century across topic categories like theatre, politics, government, shipping, etc.
