...
Historical archives of newspapers provide a wealth of information which can be used for a variety of purposes. They can be of special interest to genealogists and other library patrons for efficiently searching articles relating to people that are of interest to them across the various newspaper topics in a chronological fashion. We present a novel problem of finding influential people from a people gazetteer where the people gazetteer consists of a chain of articles related to each person entity over a historical timeline.
We use Edit distance algorithm for spelling correction of the noisy historical OCR data which is used for construction of the people gazetteer along with a novel N-gram evaluation algorithm for measuring the performance of spelling correction.
We propose the solution framework by linking each person entity with a course of events, i.e., a chain of articles that he/she is related to and rank them as influential based on ranking of their chains of articles across different newspaper topics. 
We use Named Entity Recognition for detecting the person entities in OCR archives, create chains of articles for each person by relating articles using similarity measures and LDA topic detection model and finally ranking them as influential across different topics.  
...
\chapter{Named Entity Recognition}\label{chapter:named entity recognition}
\chapter{Topic Detection}\label{chapter:topic detection} 
